...
//}
    ...
    function populateDB(tx) {
		...
		tx.executeSql('CREATE TABLE IF NOT EXISTS userlogin (id integer primary key autoincrement, user_id integer, lastlogin_time datetime, loggedin integer, name text, remberme text, uname text, pwd text)');
		 ...
		 tx.executeSql('CREATE TABLE IF NOT EXISTS wnh_emergencies (id integer primary key autoincrement, emergency_id integer DEFAULT "0", company_id integer DEFAULT "0", clinic_id integer DEFAULT "0", manager_id integer DEFAULT "0", note text, status text, filepath text, mobilefilepath text, filetype text, cdate datetime, readbymanager integer DEFAULT "0", deletebymanager integer DEFAULT "0", deletebycaompany integer DEFAULT "0", showdate text, clinic text, clinicaddress text, measurement text)');
		 tx.executeSql('CREATE TABLE IF NOT EXISTS wnh_emergency_notes (id integer primary key autoincrement, emergency_note_id integer DEFAULT "0", emergency_id integer DEFAULT "0", note text, filepath text, mobilefilepath text, filetype text, manager_id integer DEFAULT "0", company_id integer DEFAULT "0", cdate datetime, readbycompany integer DEFAULT "0", readbymanager integer DEFAULT "0", parent_id integer DEFAULT "0", notify integer DEFAULT "0")');
		 tx.executeSql('CREATE TABLE IF NOT EXISTS wnh_emergency_notifications (id integer primary key autoincrement, emergency_notification_id integer DEFAULT "0", emergency_id integer DEFAULT "0", manager_id integer DEFAULT "0", company_id integer DEFAULT "0", clinic_id DEFAULT "0", cdate datetime, readbycompany integer DEFAULT "0", readbymanager integer DEFAULT "0", notification_for text, notification_type text, notify integer DEFAULT "0")');
